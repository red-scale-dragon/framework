...
class AssetsServiceProvider extends ServiceProvider
{
	...
	private function loadAssets(string $key) {
		...
		foreach ($assets['js'] as $name => $options) {
			...
			$inFooter = array_key_exists('in_footer', $options) ? $options['in_footer'] : true;
			$useVersioning = array_key_exists('use_versioning', $options) ? $options['use_versioning'] : true;
			Asset::loadScript($name, $options['script'], $deps, $inFooter, $useVersioning);
		}
